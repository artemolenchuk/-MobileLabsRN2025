        ...
        try {
            ...
            for (const item of items) {
                ...
                try {
                    ...
                    console.warn(`useFileManager: Could not get info for item ${itemUri}:`, itemError);
                }
            }
            ...
            console.error('useFileManager: Error loading directory content:', error);
            setDirectoryContent([]);
            setUsedStorage(0);
        } finally {
            setIsLoading(false);
    ...
    useEffect(() => {
        const setupDirectory = async () => {
            try {
                ...
                console.error('useFileManager: Error setting up directory:', error);
                setIsLoading(false);
                return;
            }
    ...
    const getFileDetails = useCallback(async (uri: string, name: string) => {
        try {
            ...
            if (info.exists) {
                ...
                const details = { name, type, size, lastModified };
            ...
            console.error('useFileManager: Error getting file details:', error);
            Alert.alert('Помилка', 'Не вдалося отримати інформацію про файл.');
        }
    }, []);

    const handleDeleteItem = useCallback(async () => {
        if (!itemToDeleteUri) return;
        ...
        try {
            ...
            console.error('useFileManager: Error deleting item:', error);
    ...
    const handleCreateItem = useCallback(async (newItemName: string, isCreatingFolder: boolean) => {
        ...
        try {
            ...
            if (existingInfo.exists) {
                Alert.alert('Помилка', `Елемент з назвою "${newItemName.trim()}" вже існує.`);
                return;
            }
            ...
            console.error('useFileManager: Error creating item:', error);
            Alert.alert('Помилка створення', error.message || 'Не вдалося створити елемент.');
        }
    ...
    const handleGoUp = useCallback(() => {
        if (currentPath !== appDataDirUri && currentPath.length > appDataDirUri.length) {
            const parentPath = currentPath
                ...
                .substring(0, currentPath.substring(0, currentPath.length - 1).lastIndexOf('/') + 1);
